...
class Validator {
  ...
  public:
    ...
    std::string testset() const { return _testset; }
    ...
    void writeTestOverviewLog() {
        if (!_testOverviewLogFileName.empty()) {
            ...
            if (NULL == testOverviewLogFile)
                __testlib_fail("Validator::writeTestOverviewLog: can't test "
                               "overview log to (" +
                               fileName + ")");
            fprintf(testOverviewLogFile, "%s%s", getBoundsHitLog().c_str(),
                    getFeaturesLog().c_str());
            if (fclose(testOverviewLogFile))
                __testlib_fail("Validator::writeTestOverviewLog: can't close "
                               "test overview log file (" +
                               fileName + ")");
        }
    }
...
NORETURN void InStream::quit(TResult result, const char *msg) {
    ...
    if (strlen(msg) > maxMessageLength) {
        ...
        msg = (warn + message.substr(0, maxMessageLength - warn.length()))
                  .c_str();
    }
    ...
    if (mode != _output && result != _fail) {
        if (mode == _input && testlibMode == _validator && lastLine != -1)
            quits(_fail, std::string(msg) + " (" + name + ", line " +
                             vtos(lastLine) + ")");
        else

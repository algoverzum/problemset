...
std::vector<int> get_vals_random(std::mt19937 &rnd,
...
std::vector<int> get_vals_cyclic(const std::vector<int> &pool, int N) {
    std::vector<int> res(N);
    for (int i = 0; i < N; i++)
        res[i] = pool[i % (int)pool.size()];
    return res;
}

std::vector<std::pair<int, int>> get_random_queries(std::mt19937 &rnd, int N,
                                                    int Q) {
...
int main(int argc, char *argv[]) {
    ...
    case 0:
        ...
        vertex_vals = rnd() % 2 ? get_vals_random(rnd, values, N)
                                : get_vals_cyclic(values, N);

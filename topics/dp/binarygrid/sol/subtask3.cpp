    ...
    while (tt--) {
        ...
        for (int mask_i = 0; mask_i < (1 << n); mask_i++) {
            ...
            if (m < 2) {
                bool fail = false;
                for (int i = 2; i < n; i++) {
                    if (Get(i, 0, 0) == Get(i - 1, 0, 0) &&
                        Get(i, 0, 0) == Get(i - 2, 0, 0)) {
                        fail = true;
                        break;
                    }
                }
            ...
            for (int f1 : {0, 1}) {
                bool fail = false;
                for (int i = 2; i < n; i++) {
                    if (Get(i, 0, f1) == Get(i - 1, 0, f1) &&
                        Get(i, 0, f1) == Get(i - 2, 0, f1)) {
                        fail = true;
                        break;
                    }
                }
                ...
                for (int f2 : {0, 1}) {
                    fail = false;
                    for (int i = 2; i < n; i++) {
                        if (Get(i, 1, f2) == Get(i - 1, 1, f2) &&
                            Get(i, 1, f2) == Get(i - 2, 1, f2)) {
                            fail = true;
                            break;
                        }
                    }
                ...
                for (int f1 : {0, 1}) {
                    for (int f2 : {0, 1}) {
                        for (int f3 : {0, 1}) {
                            bool fail = false;
                            for (int i = 0; i < n; i++) {
                                if (Get(i, j, f3) == Get(i, j - 1, f2) &&
                                    Get(i, j, f3) == Get(i, j - 2, f1)) {
                                    fail = true;
                                    break;
                                }
                                if (i > 1 &&
                                    Get(i, j, f3) == Get(i - 1, j, f3) &&
                                    Get(i, j, f3) == Get(i - 2, j, f3)) {
                                    fail = true;
                                    break;
                                }
                            }

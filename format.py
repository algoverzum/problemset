...
def get_repo_root() -> Path:
    ...
    repo_root = subprocess.run(command, stdout=subprocess.PIPE, text=True, shell=True).stdout
...
def get_git_python_files() -> List[str]:
    ...
    git_files_str = subprocess.run(command, stdout=subprocess.PIPE, text=True, shell=True).stdout
...
def fmt() -> None:
    ...
    subprocess.run(["python", "-m", "black", "--line-length=120"] + python_files, cwd=root)

    cpp_files = get_git_cpp_files()
    subprocess.run(["clang-format", "-i", "--style=file:style.clang-format"] + cpp_files, cwd=root)
